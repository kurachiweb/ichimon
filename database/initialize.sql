...
CREATE TABLE IF NOT EXISTS `kurachiweb_ichimondev`.`user` (
  ...
  `password_updated_at` DATETIME NULL DEFAULT NULL COMMENT 'パスワードの最終更新日時',
...
CREATE TABLE IF NOT EXISTS `kurachiweb_ichimondev`.`category` (
  ...
  `creator_id` BIGINT UNSIGNED NOT NULL COMMENT 'カテゴリを設定したユーザーのID',
...
-- -----------------------------------------------------
-- Table `kurachiweb_ichimondev`.`user_auth`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `kurachiweb_ichimondev`.`user_auth` ;

CREATE TABLE IF NOT EXISTS `kurachiweb_ichimondev`.`user_auth` (
  `id` BIGINT UNSIGNED NOT NULL COMMENT 'メールアドレスID',
  `user_id` BIGINT UNSIGNED NOT NULL,
  `email` VARCHAR(1026) NOT NULL COMMENT 'メールアドレスの暗号化済み文字列\n復号して利用できる',
  `email_hash` VARCHAR(128) NULL COMMENT 'メールアドレスのハッシュ文字列\nメールアドレスが他のユーザーによって登録されているかの判定用',
  ...
  INDEX `fk_user_email_user_idx` (`user_id` ASC),
  UNIQUE INDEX `email_hash_UNIQUE` (`email_hash` ASC),
  CONSTRAINT `fk_user_email_user`
    FOREIGN KEY (`user_id`)
    REFERENCES `kurachiweb_ichimondev`.`user` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB
DEFAULT CHARACTER SET = ascii
COLLATE = ascii_bin
COMMENT = 'ユーザーのログイン等機密情報';
